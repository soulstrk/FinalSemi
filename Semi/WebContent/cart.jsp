<%@page import="jyi.vo.ProductsVo"%>
<%@page import="jyi.vo.CartVo"%>
...
<div class="container-fulid" style="margin-bottom: 540px;">
				...
				<table summary="번호, 사진, 제품명, 수량, 적립, 가격, 배송비, 취소">
					...
					<tbody>
						...
						<c:forEach var="vo" items="${list }">
							...
							<tr class="nbg">
								<!-- 체크박스 -->
								<td><input type="checkbox" name="basketchks"
									checked="checked" /></td>
								<c:forEach var="pvo" items="${plist }">
									...
									<c:choose>
										<c:when test="${vo.c_p_num == pvo.p_num }">
											<!-- 이미지    -->
											<td><div class="thumb">
													<a href="#"><img src="painting/o/${pvo.p_image }"
														alt="상품 섬네일" id="CartImg" /></a>
												</div></td>
											...
											<!-- 상품수량 -->
											<td><form action="cart.do?cmd=update" method="post">
													<input type="hidden" name="c_num" value="${vo.c_num }">
													<input type="hidden" name="id" value="<%=id%>"> <input
														type="text" name="c_amount" value="${vo.c_amount }"
														style="width: 50px;"> <input type="submit"
														value="수정" style="width: 50px; height: 34px;">
												</form></td>

											<!-- 상품가격 -->
											<td>${pvo.p_price}</td>
											<!-- /////////// * vo.c_amount 갯수 곱한 값을 넣어야 하는지 -->

											<!-- 상품취소 -->
											<td><a
												href="cart.do?cmd=delete&c_num=${vo.c_num }&c_id=${vo.c_id}">삭제</a></td>
 ...
 --%>
				...
				<%
					...
					ArrayList<ProductsVo> plist = (ArrayList<ProductsVo>)request.getAttribute("plist"); // 해당 상품정보
					session.setAttribute("list", list);
					session.setAttribute("plist", plist);
				%>
				<div style="float: right;">총 구매 가격: ${price }원</div>
				<br>
				<br>
				<br>
				<div class="btn-order-ctrl">
					<!-- 밑에 세개 버튼 -->
					<a href="javascript:aa();" class="checkout">상품주문하기</a> <a
						href="cart.do?cmd=delete&c_num=-1&c_id=<%=id%>" class="cart_del">장바구니
						비우기</a> <a href="cart.do?cmd=index" class="home">쇼핑계속하기</a>
