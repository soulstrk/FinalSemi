...
<form action="catroder.do" onsubmit="return checkit()">
<div class="container" style="margin : 0 20%;">
	...
	<c:forEach var ="vo" items="${list}">
		...
		<div class="col-lg-8"> <img src="painting/o/${pvo.p_image }" style=" width: 50px;height: 50px;">${pvo.p_name}</div>
		<div class="col-lg-1"> 기본</div>
		<div class="col-lg-1"><input type="hidden" name="amount" value=${vo.c_amount }> ${vo.c_amount }</div>
		<input type="hidden" name="p_num" value=${vo.c_num }>
		<div class="col-lg-1"><input type="hidden" name="p_price1" value=${vo.c_amount*pvo.p_price}> ${vo.c_amount*pvo.p_price}</div>
		
		<div class="col-lg-1"> <input type="hidden" name="sprice" value=${(vo.c_amount*pvo.p_price)*0.2}>
		
		<fmt:parseNumber value='${(vo.c_amount*pvo.p_price)*0.2}' integerOnly="true"/></div>
